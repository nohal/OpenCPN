...
void glChartCanvas::ShipDraw(ocpnDC &dc) {
  ...
  double shift_dx = 0;
  double shift_dy = 0;
  bool dynamic = m_pParentCanvas->m_animationActive ||
                 m_pParentCanvas->m_MouseDragging ||
                 m_pParentCanvas->m_chart_drag_inertia_active;
  if (m_pParentCanvas->m_bFollow && !dynamic) {
  ...
  switch (event.GetState()) {
    ...
    case GestureUpdated:
      if (g_GLOptions.m_bUseCanvasPanning) {
        ...
        if (1 || ((total_zoom_val > 1) && !m_first_zout)) {  // Zoom in
          wxPoint pinchPoint = event.GetCenterPoint();
  ...
  if (g_true_zoom) {
    ...
    if (event.IsGestureEnd()) {
      ...
      if (1 /* g_GLOptions.m_bUseCanvasPanning*/) {
        ...
        // qDebug() << "update totalzoom" << total_zoom_val << projected_scale;
        if (1 || ((m_total_zoom_val > 1) && !m_first_zout)) {  // Zoom in
          wxPoint pinchPoint = event.GetPosition();
...
void glChartCanvas::onGestureFinishTimerEvent(wxTimerEvent &event) {
  ...
  m_bforcefull = false;
  m_binPinch = false;
  m_pParentCanvas->m_inPinch = false;
  printf("******Finish\n");
}
