...
//    S57 Object Class Visibility
    s = _T ( "Going to load object filter" );
    wxLogMessage( s );
    OBJLElement *pOLE;
...
//    Fonts
    ...
    s = _T ( "Going to load TC data sources" );
    wxLogMessage( s );
//  Tide/Current Data Sources
    ...
    s = _T ( "Going to load legacy navobjects" );
    wxLogMessage( s );
//    Routes
    if( 0 == iteration ) {
        int routenum = 0;
    ...
    //    Layers
    if( 0 == iteration ) {
        s = _T ( "Going to create layer list" );
        wxLogMessage( s );
...
//      next thing to do is read tracks from the NavObject XML file,
    ...
    if( 0 == iteration ) {
        s = _T ( "Going to rotate navobjects backups" );
        wxLogMessage( s );
        ...
        if( ::wxFileExists( m_sNavObjSetChangesFile ) ) {
            s = _T ( "Going to reconstruct navobjects after crash" );
            wxLogMessage( s );
    ...
    s = _T ( "Going to load other settings" );
    wxLogMessage( s );
...
{
    wxString s = _T ( "Going to load chart dir array" );
    wxLogMessage( s );
    //    Chart Directories
...
Route *LoadGPXRoute( GpxRteElement *rtenode, int routenum, bool b_fullviz )
{
    ...
    if( g_bIsNewLayer ) pTentRoute->SetVisible( g_bLayerViz, true );
    else
