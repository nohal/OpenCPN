...
bool OCPN_Printout::OnPrintPage( int page )
{
    wxDC *dc = GetDC();
    if( dc ) {
        if( page == 1 )
...
bool OCPN_Printout::OnBeginDocument( int startPage, int endPage )
{
...
void OCPN_Printout::GetPageInfo( int *minPage, int *maxPage, int *selPageFrom, int *selPageTo )
{
    *minPage = 1;
    *maxPage = 1;
    *selPageFrom = 1;
...
bool OCPN_Printout::HasPage( int pageNum )
{
    return ( pageNum == 1 );
}

void OCPN_Printout::DrawPageOne( wxDC *dc )
{
    if( !m_cc )
        return;
    // Get the Size of the Chart Canvas
    int sx, sy;
    m_cc->GetClientSize( &sx, &sy );
...
//  And Blit/scale it onto the Printer DC
        wxMemoryDC mdc;
        mdc.SelectObject( *( m_cc->pscratch_bm ) );

        dc->Blit( 0, 0, m_cc->pscratch_bm->GetWidth(), m_cc->pscratch_bm->GetHeight(), &mdc, 0, 0 );
...
void OCPN_Printout::GenerateGLbmp( )
{
#ifdef ocpnUSE_GL
    if( g_bopengl && m_cc ) {
        int gsx = m_cc->GetglCanvas()->GetSize().x;
        int gsy = m_cc->GetglCanvas()->GetSize().y;
